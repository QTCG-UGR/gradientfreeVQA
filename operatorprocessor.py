...
class OperatorPreprocessor:
    ...
    def expand_expression(self, expr):
        ...
        for var in self.symbolic_vars:  # for a, b, c, d....
            for arg in expr.atoms(sp.exp):  # Find all exp(s) in the expression
                if var in arg.args[0].free_symbols:  # iterates over each binary variable (a, b, c...) found in exp's argument
                    f_binary_vars = [value for value in self.binary_vars if str(value)[1] == str(var)]  #ba0, ba1....
                    f_var_values = [value for value in self.var_values if str(value).startswith(str(var))] #a0, a1...
                    coefficient = arg.args[0] / var  # exp's argument numeric coefficient, ex. -0.5 in exp(-a/2)
                    expanded_part = sp.Add(*[
                        f_binary_vars[i] * sp.exp(coefficient * f_var_values[i]) for i in range(self.parts)
                    ]) 
                    
                    expanded_expr = expanded_expr.replace(arg, sp.Add(expanded_part)) # Expand all exp(x) to become: bx0*exp(x0) + bx1*exp(x1)
                    
        return expanded_expr #final_expr
    ...
    def term2expsums(self, expr):
        if isinstance(expr, sp.Mul) or isinstance(expr, sp.cosh) or isinstance(expr, sp.sinh):
            exponents = []
            ...
            if isinstance(expr, sp.cosh) or isinstance(expr, sp.sinh):
                coeff = 1
                ...
                for arg in expr.args:
                    if isinstance(arg, sp.Number):
                        coeff = arg
                    elif isinstance(arg, sp.cosh) or isinstance(arg, sp.sinh):
                        ...
                        for sub_arg in arg.args:
                            if sub_arg.is_Add and len(sub_arg.args) == 1 and \
                                (sub_arg.has(sp.cosh) or sub_arg.has(sp.sinh)):
    ...
    def expand_expressionNew(self, expr, possible_values):
...
if (RUN_UNIT_TESTS):
    ...
    for tst in tests:
        hiperb, expsums = processor.unit_test(tst, tests[tst][0], UNIT_TST_VERBOSE) 
        try:
