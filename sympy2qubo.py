...
class SympyToQUBOConverter:
    ...
    def sympy_to_qubo(self, sympy_expr, binary_vars, bqm):
        ...
        def process_expression(expr):
            ...
            if isinstance(expr, sp.Add):
                ...
                if len(binary_factors) == 1:
                    ...
                    for bf in binary_factors:
                        coeff = expr.as_coeff_Mul()[0]
                        coeff_r = coeff ** (1/len(binary_factors))
                        if isinstance(coeff_r, sp.Add):  #  Complex root.
                            coeff_r = coeff_r.args[0]  # Take real part
                        bqm.add_variable(binary_vars[str(bf)], float(coeff_r))
                        qcprint(LOG_LEVEL_DEBUG, f"  adding variable:{binary_vars[str(bf)]} with value:{float(coeff_r)}")
                        qcprint(LOG_LEVEL_DEBUG, f"  bqm.linear: {bqm.linear}")
        ...
        sympy2qubo_end = time.time()
        
        qcprint(LOG_LEVEL_DEBUG, f"Finished SymPy to QUBO conversion.  Took: {sympy2qubo_end-sympy2qubo_start} sec")
        
        #print(f"  SymPy to QUBO conversion Took {sympy2qubo_end-sympy2qubo_start} sec")
    ...
    def create_expression(self, expr_str, num_angles, num_parts):
        ...
        createexpr_end = time.time()
        
        qcprint(LOG_LEVEL_DEBUG, f"Finished Creating SymPy expression.  Took {createexpr_end-createexpr_start} sec")
        
        #print(f"  Created Expression Took {createexpr_end-createexpr_start} sec")
...
if (RUN_UNIT_TESTS):
    ...
    t_bqm, t_bvars = t_converer.unit_test('1', expr, 1, TST_PARTS, RUN_UNIT_TESTS_VERBOSE)
    try:
    ...
    expr = "ba2"
    t_bqm, t_bvars = t_converer.unit_test('1.1', expr, 1, TST_PARTS, RUN_UNIT_TESTS_VERBOSE)
    try:
    ...
    t_bqm, t_bvars = t_converer.unit_test('2', expr, 1, TST_PARTS, RUN_UNIT_TESTS_VERBOSE)
    try:
    ...
    t_bqm, t_bvars = t_converer.unit_test('3', expr, 4, TST_PARTS, RUN_UNIT_TESTS_VERBOSE)
    try:
    ...
    t_bqm, t_bvars = t_converer.unit_test('4', expr, 1, TST_PARTS, RUN_UNIT_TESTS_VERBOSE)
    try:
    ...
    expr = "ba2*bb0" 
    t_bqm, t_bvars = t_converer.unit_test('5', expr, 4, TST_PARTS, RUN_UNIT_TESTS_VERBOSE)
    try:
    ...
    t_bqm, t_bvars = t_converer.unit_test('6', expr, 1, TST_PARTS, RUN_UNIT_TESTS_VERBOSE)
    try:
