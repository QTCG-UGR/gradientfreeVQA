...
from config import COSTFUNC_THREADS
...
class QNN_train_adiabatic():
    ...
    def __init__(self, rot_angles_count, operator, discretization_parts, input_list, y_list, evovaq, num_qubits):
        ...
        self.binaryVarsinit()
        #self.angles = self.discretizeAngles(0, 1 * np.pi)
    ...
    def binaryVarsinit(self):
        # 'a', 'b', 'c', 'd', 'e', ...
        self.symbolic_vars = sp.symbols(
            ' '.join([chr(97 + i) for i in range(self.rot_angles_count)]))

        for var in self.symbolic_vars:
            # 'a0'. 'a1' ... 'an', 'b0', 'b1' ... 'bn'  n=parts
            self.var_values.extend(
                [sp.symbols(f'{var}{i}') for i in range(self.parts)])
            # python binaries: 'ba0'. 'ba1' ... 'ban', 'bb0', 'bb1' ... 'bbn'  n=parts
            self.binary_vars.extend(
                [sp.symbols(f'b{var}{i}') for i in range(self.parts)])
        ...
        self.theta_replacements = {f'θ[{i}]': str(
            self.symbolic_vars[i]) for i in range(len(self.symbolic_vars))}
    ...
    def expand_operator(self, possible_values, fixed_angles):
        ...
        operator_expanded = operator_exp.applyfunc(lambda x: self.operatorProcessor.expand_expressionNew(x, possible_values))
    ...
    def mse(self, operator, psi_0, psi_f):
        ...
        psi_f_sp_bra = psi_f_sp_immutable.H  # Hermitian conjugate of psi_f
        diff =  psi_f_sp_bra - out_vs

        diff_squared = diff.applyfunc(lambda x: x**2)
        diff_squared = diff
        out = diff_squared
        
        qcprint(LOG_LEVEL_DEBUG, f"mse finish on thread: {threading.current_thread().name}")
        qcprint(LOG_LEVEL_DEBUG, f"  mse result value: {out}")

        return out

    # Helper Method to sum binary variables based on the second character (var)
    def binary_vars_by_angle(self, angle):
        b_vars = [var for var in self.binary_vars_dimod if str(
            list(var.linear.keys())[0]).startswith('b'+str(angle))]
        return b_vars

    # Parallelize qubo formulation by item in dataset
    def buildQUBO_item(self, operator, in_data, expected_val, real_values, cqm_global):
        qcprint(LOG_LEVEL_DEBUG, f"Starting operate on thread: {threading.current_thread().name}")
        qcprint(LOG_LEVEL_DEBUG, f"S  in_data: {in_data}, expected value: {expected_val}")
        ...
        mse_evaltd = sp.expand(mse_evaltd)
        mse_evaltd = mse_evaltd.replace(lambda x: x.is_Pow and x.exp > 1, lambda x: x.base)
        ...
        # TODO: introduce auxiliary variables here
        ...
        qcprint(LOG_LEVEL_DEBUG, f"Finished building QUBO item on thread: {threading.current_thread().name}")               
    ...
    def buildQUBO(self, operator, input_list, y_list):
        ...
        if (COSTFUNC_THREADS>1): # Parallel mode execution
            with ThreadPoolExecutor(max_workers=COSTFUNC_THREADS) as executor:
                qcprint(LOG_LEVEL_INFO, f"Starting Multi Thread ({COSTFUNC_THREADS} threads) execution over Dataset entries for this training iteration")
                executor.map(lambda args: self.buildQUBO_item(*args, self.cqm), zip(operator, input_list, y_list, real_values))
              
        else:  # Single Thread execution
            for i in range(len(input_list)):
                self.buildQUBO_item(operator, input_list[i], y_list[i], real_values, self.cqm)
        ...
        for label, constraint in self.cqm.constraints.items():
            qcprint(LOG_LEVEL_INFO, f"   constraint {label}: {constraint}")
    ...
    def updateQUBO(self, qubo_item):
        ...
        for var, bias in qubo_item.linear.items():
            if var in self.cqm.objective.linear:
                old_val = self.cqm.objective.get_linear(var)
                self.cqm.objective.add_linear(var, old_val+bias)
            else:
                self.cqm.objective.add_linear(var, bias)
        
        # Add quadratic biases from bqm2 to bqm1
        for (var1, var2), bias in qubo_item.quadratic.items():
            if (var1, var2) in self.cqm.objective.quadratic:
                old_val = self.cqm.objective.get_quadratic(var1, var2)
                self.cqm.objective.add_quadratic(var1, var2, old_val +bias)
            else:
                self.cqm.objective.add_quadratic(var1, var2, bias)
    ...
    def train(self, deep_levels):
        ang_values = {str(var): 0.0 for var in self.symbolic_vars}
        ...
        last_accuracy = 0.0
        abortTraining = False
        for deepl in range (deep_levels): # 3 niveles de profundidad, max 7 iteraciones (1+2+3) 
            if abortTraining:
                break
            
            qcprint(LOG_LEVEL_INFO, f"\nDeep level: {deepl+1}. Segments: {2**(deepl+1)}")
            seg_size = range_end / self.parts**(deepl+1)
            qcprint(LOG_LEVEL_INFO, f"Segment size = {seg_size}")
            for seg_num in range  (self.parts**(deepl+1)): # 2**i rangos de búsqueda potencial.  Ej 8 para i = 3
                total_iterations += 1
                ...
                lp = (seg_end - seg_start)/self.parts
                ...
                angs_optimized, iter_accuracy = self.train_iter(seg_start, seg_end, angs2optimize, possible_values, angs_fixed)
                training_iterations +=1
                qcprint(LOG_LEVEL_INFO, f" Optimized angles after iteration: {angs_optimized}")
                
                for ang in ang_values:
                    if ang in angs_optimized:
                        ang_values[ang] = angs_optimized[ang]
                
                if (ADIABATIC_TRAIN_ITER_ACCURACY):
                    qcprint(LOG_LEVEL_INFO, f" Iteration Accuracy: {iter_accuracy}.  Previous accuracy : {last_accuracy}")
                    if training_iterations>1 and iter_accuracy < last_accuracy:
                        if last_accuracy - iter_accuracy > ADIABATIC_TRAIN_ITER_TOLERANCE:
                            qcprint(LOG_LEVEL_INFO, f" Iteration Accuracy is bellow threshold {ADIABATIC_TRAIN_ITER_TOLERANCE}.  Abort training.")
                            abortTraining = True
                            break
                    else:
                        last_accuracy = iter_accuracy
                        
        training_end = time.time()
        qcprint(LOG_LEVEL_INFO, f"\nTotal Iterations: {total_iterations}.  Training Iterations: {training_iterations}.")
        qcprint(LOG_LEVEL_INFO, f"Angle values after optimization: {ang_values}")
        qcprint(LOG_LEVEL_INFO, f"Training took: {training_end-training_start} seconds")
    ...
    def train_iter(self, range_start, range_end, angs2optimize, possible_values, angs_fixed):
        ...
        blp = self.buildQUBO(operator_expanded, self.input_list, self.y_list) # Initialize empty Summation
        
        optimal = []
        solver = dimod.ExactCQMSolver()
        if ADIABATIC_REAL:  # Real HW Training
            qcprint(LOG_LEVEL_INFO, "Training on real solver")
            client = Client.from_config(token=DWAVE_TOKEN)
            solver = client.get_solver(supported_problem_types__issubset={'cqm'})
            start_time = time.time()
            computation = solver.sample_cqm(blp, label="experiment_" + str(self.parts) + "_parts", time_limit=DWAVE_TIMELIMIT)
            end_time = time.time()
            ...
            qcprint(LOG_LEVEL_INFO, "Training on simulated solver")
            start_time = time.time()
            solutions = solver.sample_cqm(blp)
            end_time = time.time()

            qcprint(LOG_LEVEL_INFO, f"Total solutions: {len(solutions)}")
            feasible_sols = solutions.filter(lambda s: s.is_feasible)
            qcprint(LOG_LEVEL_INFO, f"Feasible solutions: {len(feasible_sols)}")
            if len(feasible_sols.samples()) > 0:
                optimal = feasible_sols.samples()[0]
            else:
                qcprint(LOG_LEVEL_INFO, "No feasible solution found")

        optimal = {var: value for var, value in optimal.items() if value == 1}
        for opt in optimal:
        ...
        if (ADIABATIC_TRAIN_ITER_ACCURACY):
            ...
            iteration_accuracy = self.evovaq.test(list(optimal_sorted.values()))
        ...
        if (ADIABATIC_TRAIN_ITER_ACCURACY):
            qcprint(LOG_LEVEL_DEBUG, f"Iteration Accuracy: {iteration_accuracy}")
        
        return result_angs2optimize, iteration_accuracy
